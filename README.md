if IY_LOADED and not _G.IY_DEBUG == true then
    -- error("Erm what the sigma? GET OUT NIGGA!", 0)
    return
end
...
if not game:IsLoaded() then
    ...
    notLoaded.Text = "drake is coming for you having fun time"
    game.Loaded:Wait()
    notLoaded:Destroy()
end
...
PluginsHint.Text = "Ishowspeed ass leaked (https://www.youtube.com/watch?v=dQw4w9WgXcQ)"
...
invisRunning = false
addcmd('invisible',{'ivs'},function(args, speaker)
...
addcmd("gra", {"grav"}, function(args, speaker)
...
addcmd("hh", {"hheight"}, function(args, speaker)
...
addcmd("ly", {"laydown"}, function(args, speaker)
	local humanoid = speaker.Character:FindFirstChildWhichIsA("Humanoid")
	humanoid.Sit = true
	task.wait(0.1)
...
addcmd('animation',{'anim'},function(args, speaker)
	if not r15(speaker) then
		...
		notify('Nigga think he can'','Nigga you need r6 bitch ass nigga')
	end
end)
...
addcmd('speed',{'ws','wdd'},function(args, speaker)
	if args[2] then
...
addcmd('audiologger',{'alogger'},function(args, speaker)
	notify("Loading",'Hold on man!')
	loadstring(game:HttpGet(('https://raw.githubusercontent.com/infyiff/backup/main/audiologger.lua'),true))()
end)

addcmd('Scpth',{'Scripthubv3'},function(args, speaker)
	notify("Loading",'Hold on Nigga')
...
addcmd('chat',{'sy'},function(args, speaker)
...
addcmd('sexa',{},function(args, speaker)
	for i,v in pairs(speaker.Character:GetDescendants()) do
...
addcmd('chatlogs',{'clog'},function(args, speaker)
	join.Visible = false
	chat.Visible = true
	table.remove(shade3,table.find(shade3,selectChat))
	table.remove(shade2,table.find(shade2,selectJoin))
...
addcmd('invfling',{},function(args, speaker)
...
addcmd('tk', {'teleportwalk'}, function(args, speaker)
	tpwalking = true
...
addcmd('untk', {'unteleportwalk'}, function(args, speaker)
	tpwalking = false
end)
...
function bring(speaker,target,fast)
	if tools(speaker) then
		...
		notify('Tool Required','you need tools NIGGA!')
	end
end
...
function teleport(speaker,target,target2,fast)
	if tools(speaker) then
		...
		notify('MrNigga tool?','NIGGA YOU NEED TOOLS NIGGA WHAT THE FACK NIGGA')
	end
end
...
addcmd('hitbx',{},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players) do
